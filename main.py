...
def processUploadFiles(filename,filesize,files,update,bot,message,thread=None,jdb=None):
    try:
        ...
        if cloudtype == 'moodle':
            ...
            bot.editMessageText(message,'》☁️Subiendo a la Nube...☁️')
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if '/add' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    jdb.save()
                    msg = 'El usuario @'+user+' ahora tiene acceso al bot👍'
                    bot.sendMessage(update.message.chat.id,msg)
                except:
                    bot.sendMessage(update.message.chat.id,'❌Error en el comando /adduser username❌')
            else:
                bot.sendMessage(update.message.chat.id,'❌No posee permiso para realizar esta acción❌')
            return
        if '/ban' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
                try:
                    ...
                    jdb.remove(user)
                    jdb.save()
                    msg = 'El usuario @'+user+' ha sido baneado❌'
                    bot.sendMessage(update.message.chat.id,msg)
                except:
                    bot.sendMessage(update.message.chat.id,'❌Error en el comando /banuser username❌')
            else:
                bot.sendMessage(update.message.chat.id,'❌No tiene permiso para realizar esta acción❌')
            return
        if '/getdb' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
        ...
        if '/zip' in msgText:
            ...
            if getUser:
                try:
                   ...
                   jdb.save()
                   msg = 'A partir de ahora los zips serán de '+ sizeof_fmt(size*1024*1024)+' de tamaño.'
                   bot.sendMessage(update.message.chat.id,msg)
                except:
        ...
        if '/start' in msgText:
            ...
            #    bot_token = '5270206587:AAFT51XOQ8qLHqdUIm86wRnE8iKF2NGMQyA'
...
def main():
    bot_token = os.environ.get('bot_token')
    #set in debug
    bot_token = '5177417474:AAHd12emu1Vx37ZaDsaTwABf1cjalXtXqpM'
